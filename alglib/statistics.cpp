...
void samplemoments(const real_1d_array &x, const ae_int_t n, double &mean,
	...
	alglib_impl::samplemoments(const_cast<alglib_impl::ae_vector*>(x.c_ptr()),
			n, &mean, &variance, &skewness, &kurtosis, &_alglib_env_state);
...
void spearmancorrm(const real_2d_array &x, const ae_int_t n, const ae_int_t m,
	...
	alglib_impl::spearmancorrm(const_cast<alglib_impl::ae_matrix*>(x.c_ptr()),
			n, m, const_cast<alglib_impl::ae_matrix*>(c.c_ptr()),
			&_alglib_env_state);
	alglib_impl::ae_state_clear(&_alglib_env_state);
	return;
}
...
void wilcoxonsignedranktest(const real_1d_array &x, const ae_int_t n,
		const double e, double &bothtails, double &lefttail, double &righttail,
		const xparams _xparams) {
	jmp_buf _break_jump;
	alglib_impl::ae_state _alglib_env_state;
	alglib_impl::ae_state_init(&_alglib_env_state);
...
void onesamplesigntest(const real_1d_array &x, const ae_int_t n,
	...
	alglib_impl::onesamplesigntest(
			const_cast<alglib_impl::ae_vector*>(x.c_ptr()), n, median,
...
void spearmanrankcorrelationsignificance(const double r, const ae_int_t n,
	...
	if (_xparams.flags != 0x0)
		ae_state_set_flags(&_alglib_env_state, _xparams.flags);
	alglib_impl::spearmanrankcorrelationsignificance(r, n, &bothtails,
			&lefttail, &righttail, &_alglib_env_state);
	alglib_impl::ae_state_clear(&_alglib_env_state);
	return;
}
...
void studentttest1(const real_1d_array &x, const ae_int_t n, const double mean,
	...
	alglib_impl::studentttest1(const_cast<alglib_impl::ae_vector*>(x.c_ptr()),
			n, mean, &bothtails, &lefttail, &righttail, &_alglib_env_state);
	alglib_impl::ae_state_clear(&_alglib_env_state);
	return;
}
...
void studentttest2(const real_1d_array &x, const ae_int_t n,
	...
	alglib_impl::studentttest2(const_cast<alglib_impl::ae_vector*>(x.c_ptr()),
			n, const_cast<alglib_impl::ae_vector*>(y.c_ptr()), m, &bothtails,
			&lefttail, &righttail, &_alglib_env_state);
	alglib_impl::ae_state_clear(&_alglib_env_state);
	return;
}
...
void onesamplevariancetest(const real_1d_array &x, const ae_int_t n,
		const double variance, double &bothtails, double &lefttail,
		double &righttail, const xparams _xparams) {
	jmp_buf _break_jump;
	alglib_impl::ae_state _alglib_env_state;
	alglib_impl::ae_state_init(&_alglib_env_state);
...
ae_bool _trypexec_basestat_rankdatabasecase(/* Real    */ae_matrix *xy,
		ae_int_t i0, ae_int_t i1, ae_int_t nfeatures,
		ae_bool iscentered, apbuffers *buf0, apbuffers *buf1, ae_state *_state);
...
static void jarquebera_jarqueberastatistic(/* Real    */ae_vector *x,
		ae_int_t n, double *s, ae_state *_state);
...
void samplepercentile(/* Real    */ae_vector *x, ae_int_t n, double p,
	...
	*v = 0;
	...
	ae_assert(
			ae_fp_greater_eq(p, (double) (0)) && ae_fp_less_eq(p, (double) (1)),
...
/* Real    */ae_matrix *c, ae_state *_state) {
	...
	for (i = 0; i <= m - 1; i++) {
		v = t.ptr.p_double[i];
		for (j = 0; j <= m - 1; j++) {
			c->ptr.pp_double[i][j] = c->ptr.pp_double[i][j] * v
					* t.ptr.p_double[j];
		}
	}
	ae_frame_leave(_state);
...
/* Real    */ae_matrix *c, ae_state *_state) {
	...
	for (i = 0; i <= n - 1; i++) {
		ae_v_addd(&t.ptr.p_double[0], 1, &x->ptr.pp_double[i][0], 1,
				ae_v_len(0, m1 - 1), v);
		for (j = 0; j <= m1 - 1; j++) {
			samex.ptr.p_bool[j] = samex.ptr.p_bool[j]
					&& ae_fp_eq(x->ptr.pp_double[i][j], x0.ptr.p_double[j]);
		}
	}
	for (i = 0; i <= n - 1; i++) {
	...
	for (i = 0; i <= n - 1; i++) {
		ae_v_addd(&t.ptr.p_double[0], 1, &y->ptr.pp_double[i][0], 1,
				ae_v_len(0, m2 - 1), v);
		for (j = 0; j <= m2 - 1; j++) {
			samey.ptr.p_bool[j] = samey.ptr.p_bool[j]
	 ...
	 */
	rmatrixgemm(m1, m2, n, (double) 1 / (double) (n - 1), x, 0, 0, 1, y, 0, 0,
			0, 0.0, c, 0, 0, _state);
	ae_frame_leave(_state);
}
...
/* Real    */ae_matrix *c, ae_state *_state) {
	...
	for (i = 0; i <= n - 1; i++) {
		...
		for (j = 0; j <= m1 - 1; j++) {
			...
			sx.ptr.p_double[j] = sx.ptr.p_double[j]
	...
	for (i = 0; i <= n - 1; i++) {
		...
		for (j = 0; j <= m2 - 1; j++) {
			...
			sy.ptr.p_double[j] = sy.ptr.p_double[j]
...
/* Real    */ae_matrix *c, ae_state *_state) {
	 ...
	 * calculate cross-covariance matrix
	 */
	rmatrixgemm(m1, m2, n, (double) 1 / (double) (n - 1), &xc, 0, 0, 0, &yc, 0,
			0, 1, 0.0, c, 0, 0, _state);
...
void rankdata(/* Real    */ae_matrix *xy, ae_int_t npoints, ae_int_t nfeatures,
	...
	basecasecost = 10000;
	if (ae_fp_less(
			rmul3((double) (npoints), (double) (nfeatures),
...
/*************************************************************************
 Serial stub for GPL edition.
 *************************************************************************/
ae_bool _trypexec_rankdatacentered(/* Real    */ae_matrix *xy, ae_int_t npoints,
		ae_int_t nfeatures, ae_state *_state) {
	return ae_false;
}
...
static void basestat_rankdatarec(/* Real    */ae_matrix *xy, ae_int_t i0,
	...
	if (i1 - i0 >= 4
			&& ae_fp_greater_eq(
					rmul3((double) (i1 - i0), (double) (nfeatures),
							logbase2((double) (nfeatures), _state), _state),
...
static void basestat_rankdatabasecase(/* Real    */ae_matrix *xy, ae_int_t i0,
		ae_int_t i1, ae_int_t nfeatures,
		ae_bool iscentered, apbuffers *buf0, apbuffers *buf1,
		ae_state *_state) {
	ae_int_t i;
	...
	*bothtails = ae_minreal(
			2
					* binomialdistribution(
							ae_minint(gtcnt, necnt - gtcnt, _state), necnt, 0.5,
							_state), 1.0, _state);
...
/* Real    */ae_vector *y, ae_int_t m, double *bothtails, double *lefttail,
	...
	*righttail = 0;
	...
	if (n + m > 2) {
		...
		s = ae_sqrt(
				s * ((double) 1 / (double) n + (double) 1 / (double) m)
						/ (n + m - 2), _state);
	}
...
/* Real    */ae_vector *y, ae_int_t m, double *bothtails, double *lefttail,
	...
	*righttail = 0;
	...
	if (ae_fp_greater(stat, (double) (0))) {
		p = 1
				- 0.5
						* incompletebeta(df / 2, 0.5,
								df / (df + ae_sqr(stat, _state)), _state);
	} else {
		p = 0.5
				* incompletebeta(df / 2, 0.5, df / (df + ae_sqr(stat, _state)),
						_state);
	}
	*bothtails = 2 * ae_minreal(p, 1 - p, _state);
	*lefttail = p;
...
/* Real    */ae_vector *y, ae_int_t m, double *bothtails, double *lefttail,
	...
	*righttail = 0;
	...
	sigma = ae_sqrt(
			rmul2((double) (n), (double) (m), _state) / ns / (ns - 1) * tmp,
			_state);
	s = (u - mu) / sigma;
	if (ae_fp_less_eq(s, (double) (0))) {
		...
		mp = 1
				- ae_exp(
						mannwhitneyu_usigma(-(u - 1 - mu) / sigma, n, m,
								_state), _state);
	} else {
		mp = ae_exp(mannwhitneyu_usigma((u - mu) / sigma, n, m, _state),
				_state);
		p = 1
				- ae_exp(
						mannwhitneyu_usigma((u + 1 - mu) / sigma, n, m, _state),
						_state);
	}
	*lefttail = boundval(ae_maxreal(mp, 1.0E-4, _state), 0.0001, 0.2500,
			_state);
	*righttail = boundval(ae_maxreal(p, 1.0E-4, _state), 0.0001, 0.2500,
			_state);
	*bothtails = 2 * ae_minreal(*lefttail, *righttail, _state);
	ae_frame_leave(_state);
}
...
static double mannwhitneyu_usigma(double s, ae_int_t n1, ae_int_t n2,
	...
	if (ae_minint(n1, n2, _state) == 5) {
		...
		if (ae_maxint(n1, n2, _state) > 29) {
			...
			result = mannwhitneyu_uninterpolate(f0, f1, f2,
...
/* Real    */ae_vector *y, ae_int_t m, double *bothtails, double *lefttail,
	...
	*bothtails = 1
			- (fdistribution(df1, df2, 1 / stat, _state)
...
void onesamplevariancetest(/* Real    */ae_vector *x, ae_int_t n,
		double variance, double *bothtails, double *lefttail, double *righttail,
		ae_state *_state) {
	ae_int_t i;
	double xmean;
	double xvar;
